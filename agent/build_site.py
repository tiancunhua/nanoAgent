...
LESSONS = [
    ...
    Lesson(
        ...
        demo_command='python agent/03-skills-mcp/agent-skills-mcp.py "扫描项目里所有 TODO，调用 project_guide 说明 Rule、Skill、MCP 如何接入，并生成最多 3 项修复顺序"',
        demo_goal="展示 Agent 如何在启动时加载一个最小 Rule、一个最小 Skill 与一个 MCP 工具，并把这些外置能力带入本轮任务。",
        demo_expected=[
            "终端先出现 `[Init] Loading ClaudeCode features...`，随后打印 `[Rules] Loaded 1`、`[Skills] Loaded 1 skills: todo_prioritizer`、`[MCP] Loaded 1 MCP tools: project_guide`。",
            "模型可以调用 `[Tool] project_guide(...)`，这说明 MCP 工具已经从 `.agent/mcp.json` 并入工具列表。",
            "本讲将 `DEFAULT_MAX_ITERATIONS` 提到 10，让现场演示有足够轮次完成搜索、调用 MCP 工具与生成结论。",
            "主线一句话：Rule 与 Skill 进入 prompt，MCP 进入 tools，三者接入位置不同。",
        ],
        student_takeaways=[
